...
const { isLoading } = useQuery(
    ...
    async () => {
      let url = `/inventory?project=${project}&sector=${sector}&pageSize=${pageSize}&status=assigned`;
    ...
    <div className="relative form-row" ref={dropdownRef}>
      ...
      <div className={`relative w-full rounded-lg border h-[32px] transition-all duration-300 ${containerClasses}`}>
        <input
          ...
          className="w-full py-3 px-4 h-full text-sm text-primary bg-transparent rounded-lg outline-none"
          placeholder=""
        />
