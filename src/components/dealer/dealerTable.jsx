...
const DealerTable = ({ data, currentPage, setCurrentPage, limit, setLimit, totalCount }) => {
...
<div className=' flex flex-col  sm:flex-row sm:justify-between  w-[95%] mx-auto '>
  ...
  <p className="text-dark1 font-semibold">All Dealers</p> 

  </div>

</div>      {/* Table Window Scrollable */}
      <div className="  overflow-x-auto maintable mt-4  ">
        <table className="">
          <thead className="">
            <tr>
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {data.map((row) => (
              <tr key={row._id} 
                ...
                <td className=""><Status status={formatLabel(row.status)}/>
                    
                    </td>
                <td className="">
                  <div className="flex flex-row gap-2 justify-center items-center">
                    {/* Edit Icon Button */}
                    <button onClick={()=>    navigate(`/app/dealer/edit/${row._id}`, { state: row })}
                      className="w-[22px] h-[22px] flex items-center justify-center rounded-md text-white bg-primary cursor-pointer"
                    >
                        <EyetSVG/>
                    </button>
