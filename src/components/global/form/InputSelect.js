...
const InputSelect = (props) => {
  const { label, name, formik, type = "text",placeholder, options = [], ...rest } = props;
  ...
  // FORM VALUE STORED IN FORMIK (always the option.value)
  const storedValue = formik.values[name];

  const isError = formik.touched[name] && formik.errors[name];
  const isActive = focused || storedValue;

  // Find the label (key) for the selected item
  const selectedLabel = (() => {
  ...
  const handleSelect = (option) => {
    const actualValue = option.value ?? option;     // stored in formik
  ...
  return (
    <div className="relative form-row" ref={dropdownRef}>
      ...
      >
        {/* Input (shows key but stores value) */}
        <input
          id={name}
          name={name}
          type="text"
