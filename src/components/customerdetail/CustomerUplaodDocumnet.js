...
limitDocument,
 ...
 const dispatch=useDispatch();
   ...
   const { docCustomerDocumnet } = useSelector(
     (state) => state.customer
   );
  const {user } = useSelector((state) => state.auth);


const start = (currentDocumentPage - 1) * limitDocument + 1;
const end = start + docCustomerDocumnet.length - 1;
 ...
 <div className="bg-white rounded-xl flex flex-col gap-5">
  ...
  {docCustomerDocumnet.map((row, index) => (
              <tr key={row?._id}
              
               className={`border-b hover:bg-gray-50 transition ${
        docCustomerDocumnet.includes(row?._id) ? "bg-[#F0FAFF]" : ""
      }`}
              >
                ...
                <td className="px-3 py-4">
                  ...
                  {row?.status ==='deleted' ?  (
                   ...
                   onClick={() => {
                  setVeiwDetail(true);
                  dispatch(setCustomerDocumnet(row));
                }}
...
<p className="text-sm text-[#A8A8A8]">
  Showing {totalCustomerDocs === 0 ? 0 : start} to {totalCustomerDocs === 0 ? 0 : end} of {totalCustomerDocs} entries
</p>
                             <div className="w-[140px] h-[40px] ">
                         <PageLimit totalpages={totalCustomerDocs || 10} limit={limitDocument} setLimit={setLimitDocument} />
                            </div>
                      </div>
...
export default CustomerUplaodDocumnet;