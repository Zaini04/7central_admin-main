...
const ProjectInput = ({ name, placeholder = "Select Project", onSelect }) => {
...
const { data, isLoading } = useQuery({
  ...
  onSuccess: (res) => {
    const { data: { data: { docs } } } = res;
    setProjects(docs);
    setFilteredOptions(docs);
  },
});
...
/>
  ...
  <div className="px-3 py-2 text-gray-400">No Project Found</div>
) : (
  filteredOptions.map((option) => (
    <div
      key={option._id}
      onClick={() => handleSelect(option)}
      className="px-3 py-2 cursor-pointer  text-sm  md:text-base hover:bg-gray-100 text-gray-700"
    >
      {option.title}
    </div>
  ))
