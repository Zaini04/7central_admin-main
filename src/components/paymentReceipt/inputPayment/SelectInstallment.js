...
const SelectInstallments = ({ label = " Installments", onSelect }) => {
...
function formatLabel(text = "") {
  ...
  const queryKey = ["fetch-all-installments", keyword,];

  const { isLoading, isError, error } = useQuery(
    queryKey,
    () => {
      let url = `/sale/get-all-installments?pageSize=${pageSize}`;
  ...
  return (
    <div className="relative form-row " ref={dropdownRef}>
      ...
      >
        <input
          type="text"
          value={formatLabel(selectedLabel)}
          onFocus={() => {
            setFocused(true);
            setShowMenu(true);
          }}
