...
const InputDate = (props) => {
  ...
  const displayDate = formattedValue
  ? format(formattedValue, 'dd/MMM/yyyy')
  : '';
  ...
  return (
    <div className="relative form-row" ref={wrapperRef}>
      ...
      <div className={`flex items-center border rounded-lg relative ${containerClasses}`}>
        <FaCalendarAlt className="text-gray-400 z-0 absolute right-3" />
        ...
        <label
          htmlFor={name}
          className={`absolute left-0  bg-white px-1 transition-all duration-300 pointer-events-none
            ${
              isActive
                ? `top-[-12px] text-[13px] ${
                    isError
