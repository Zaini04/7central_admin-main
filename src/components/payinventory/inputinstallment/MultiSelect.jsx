...
const MultiSelect = ({ label, name,placeholder, options = [], formik, onSelect, onRemove }) => {
  ...
  return (
    <div className="relative form-row w-full pb-10" ref={dropdownRef}>
      <div placeholder={placeholder} className="relative h-[32px] w-full transition-all duration-300   rounded-lg border border-lighter">
        {/* Selected items display */}
        <div
          className="flex flex-wrap gap-2 py-2 px-3 cursor-pointer min-h-[46px]"
        ...
        >
          {selectedValues.length > 0 &&
            selectedValues.map((value) => (
              <span
                key={value}
                className="bg-primary h-6 text-white text-[10px] rounded-full px-2 py-0.5 mt-1.5 font-semibold flex items-center gap-1"
              >
                {getLabel(value)}
                <button
                  type="button"
