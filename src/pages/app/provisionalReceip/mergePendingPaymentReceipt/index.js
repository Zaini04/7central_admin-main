...
const MergePendingPaymentReceipt = () => {
  ...
  const currentPage = 1;
  const limit = 200;
  const status = "pending"; // only fetch pending payments for merging
  ...
  const { data, isLoading, isError, error, refetch, isFetching } = useQuery(
    ["fetch-pending-payments-receipts", selectedInventory, currentPage, limit, status],
    async () => {
      const url =
      ...
      <Formik
        ...
        onSubmit={async (values, { setSubmitting, resetForm }) => {
          try {
            ...
            const res = await Axios.post("/payment/merge-pending", {
              ...
              <div className="sm:col-span-2">
                ...
                {selectedInventory && !isLoading && paymentsDocs.length === 0 ? (
                  <div className="flex items-center gap-3 mt-2">
                    <p className="text-gray-500 text-xs">No pending payments found for this inventory.</p>
                    <button
                      type="button"
...
export default MergePendingPaymentReceipt;